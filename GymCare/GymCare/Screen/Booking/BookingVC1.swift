...
//  GymCare
//
//  Created by Truong Nguyen Huu on 21/05/2023.
//

import UIKit

class BookingVC1: BaseViewController {
    ...
    @IBOutlet private weak var toDatePickerView: InputFieldView!
    ...
    private func configUI() {
        ...
        toDatePickerView.editingCallBack = { value in
            if let schedule = self.schedule {
                let money = castToInt(schedule.scheduleClass?.money) * castToInt(value)
                self.moneyLabel.text = "Giá dự kiến: " + castToString(money).formatMoney() + " VNĐ"
            } else {
                let money = castToInt(self.address.addressClass?.money) * castToInt(value)
                self.moneyLabel.text = "Giá dự kiến: " + castToString(money).formatMoney() + " VNĐ"
            }
        }
...
//        fromDatePickerView.editingIsAllow = false
        ...
        let endDate = formatDateString(dateString: castToString(schedule.end_date), Constants.DATE_PARAM_FORMAT, Constants.DATE_FORMAT)
        let numberOfMonth = viewModel.getSumMonth(fromDate: castToString(fromDatePickerView.value),
                                                  toDate: castToString(endDate))
        toDatePickerView.value = castToString(numberOfMonth)
    ...
    @IBAction private func onClickSubmit(_ sender: UIButton) {
        ...
        let toDate = viewModel.getToDate(fromDateStr: castToString(fromDatePickerView.value),
                                         numberOfMonth: castToInt(toDatePickerView.value))
